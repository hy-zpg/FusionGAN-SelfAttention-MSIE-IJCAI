        ...
        if self.matching > 0:
            ...
            with tf.variable_scope(self.name, reuse=self.reuse):
                ...
                with tf.variable_scope('g_tanh'):
                    ...
                    # print(compare_image) (1, 336, 252, 3)
                    # print(compare_image)
                    # images_name = tf.cond(z1z2_training, lambda: 'reconstruction', lambda: 'gan')
                    # images_name =  tf.cast(images_name,dtype=tf.string)
                    # tf.summary.image('gan', compare_image)
                    # store_name = tf.cond(z1z2_training, lambda: 'reconstruction', lambda: 'gan')
                    # tf.summary.image(str(store_name), compare_image)
                    # images_name = tf.cond(z1z2_training, lambda: similarities, lambda: similarities_real)
                    tf.summary.image('comparison', compare_image)

                # with tf.variable_scope('reconstruction_layer',reuse=self.reuse):
                    # S_l1 = []
                    # for s in range(support_num):
                    #     S_l1.append(tf.reduce_mean(tf.abs(gan_decoder - support_input[:, s]), axis=[2, 3]))
                    # S_l1 = tf.stack(S_l1, axis=1)
                    # S_l1 = self.classify(similarities, support_set_y=S_l1, training=training,
                    #                      name='classify')
                ...
                # with tf.variable_scope('reconstruction_layer'):
                # ##### feature
                # s_encode_reconstruction = []
                # for s in range(support_num):
                #     s_encode, _ = self.unet_encoder(image_input=gan_decoder, training=training,
                #                                     dropout_rate=dropout_rate, scope='unet_S')
                #     s_code_reconstruction_current = tf.reduce_mean(tf.abs(s_encode - prototype_feature[:, s]),
                #                                                    axis=[2, 3])
                #     s_encode_reconstruction.append(s_code_reconstruction_current)

                # s_encode_reconstruction = tf.stack(s_encode_reconstruction, axis=1)
                # reconstruction_feature_S = self.classify(similarities,
                #                                          support_set_y=s_encode_reconstruction, training=training,
                #                                          name='classify')
                # reconstruction_feature_S = tf.reduce_mean(reconstruction_feature_S)

                # b_encode, _ = self.unet_encoder(image_input=gan_decoder, training=training,
                #                                 dropout_rate=dropout_rate, scope='unet_B')
                # reconstruction_feature_B = self.reconstruction_cost(b_encode, xb_reconstruction)
                ##### images
                # reconstruction_loss = self.reconstruction_cost(gan_decoder, conditional_input)

            # with tf.variable_scope('few_shot_classifier'):
            #     f_encoded_real = self.Matching_classifier(image_input=conditional_input, training=training,
            #                                               dropout_rate=dropout_rate)
            #     f_encoded_fake = self.Matching_classifier(image_input=gan_decoder, training=training,
            #                                               dropout_rate=dropout_rate)

            #     similarities_real, _, _ = self.dn(support_set=matching_feature, input_image=f_encoded_real,
            #                                       training=training, name='dn')
            #     similarities_fake, _, _ = self.dn(support_set=matching_feature, input_image=f_encoded_fake,
            #                                       training=training, name='dn')

            #     preds_real = self.classify(similarities_real,
            #                                support_set_y=y_support_b, training=training, name='classify')

            #     if len(preds_real.get_shape()) < 2:
            #         preds_real = tf.expand_dims(preds_real, -1)
            #     crossentropy_loss_real = self.crossentropy_softmax(targets=y_batch_a, outputs=preds_real)
            #     correct_prediction_real = tf.equal(tf.argmax(preds_real, 1), tf.argmax(y_batch_a, 1))
            #     accuracy_real = tf.reduce_mean(tf.cast(correct_prediction_real, tf.float32))

            #     preds_fake = self.classify(similarities_fake,
            #                                support_set_y=y_support_b, training=training, name='classify')
            #     if len(preds_fake.get_shape()) < 2:
            #         preds_fake = tf.expand_dims(preds_fake, -1)
            #     crossentropy_loss_fake = self.crossentropy_softmax(targets=y_batch_a, outputs=preds_fake)
            #     fake_category = tf.argmax(preds_fake, 1)
            #     confidence_score = tf.reduce_max(preds_fake, 1)

            #     correct_prediction_fake = tf.equal(tf.argmax(preds_fake, 1), tf.argmax(y_batch_a, 1))
            #     accuracy_fake = tf.reduce_mean(tf.cast(correct_prediction_fake, tf.float32))

            #     if self.print:
            #         crossentropy_loss_fake = tf.Print(crossentropy_loss_fake, [crossentropy_loss_fake],
            #                                           'loss_few_shot_classification')
            #     # loss_few_shot_classification = crossentropy_loss_fake
            #     loss_few_shot_classification = tf.zeros([self.batch_size])
...
class Discriminator:
    ...
    def __call__(self, input_images1, input_images2, input_images3,  sim_label, similarities_index, y_global_batch, y_global_support, selected_classes,
        ...
        with tf.variable_scope(self.name, reuse=self.reuse):
            ...
            ##### mode loss
            mode_loss = tf.reduce_mean(tf.abs(feature_fake1 - feature_fake2)) / tf.reduce_mean(tf.abs(similarities1 - similarities2))
            ...
            sim_loss1 = []
            for i in range(support_num):
                similarities = tf.expand_dims(similarities1[:,i],1)
                similarities = tf.expand_dims(similarities,1)
                similarities = tf.tile(similarities, [1, feature_fake1.get_shape()[1], feature_fake1.get_shape()[2],  1])
                real_current = feature_real_total[i]
                feature_current = tf.concat([real_current, similarities, feature_fake1],axis=3)
                feature_current = lrelu(feature_current, 0.2)
                feature_current_avg = tf.reduce_mean(feature_current, axis=[1, 2])
                sim_dense = tf.layers.dense(feature_current_avg, units=1,name='predict_sim')
                sim_dense = tf.nn.sigmoid(sim_dense)
                sim_loss1.append(tf.square(sim_dense - similarities1[:,i]))
