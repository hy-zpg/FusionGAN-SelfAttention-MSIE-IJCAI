    ...
    def get_batch(self, dataset_name):
        ...
        if self.is_training > 0:
            for i in range(self.batch_size):
                # choose_classes = np.random.choice(len(self.datasets[dataset_name][:self.selected_classes]), size=self.selected_classes)
                # choose_classes = [(i*self.selected_classes+j) for j in range(self.selected_classes)]
                for j in range(self.selected_classes):
                    index1 = np.array([k for k in range(0,testing_num)])
                    index2 = np.array([k for k in range(testing_num, self.datasets[dataset_name][j].shape[0])])
                    # index2 = np.array([k for k in range(testing_num, 35)])
                    choose_samplesa = np.random.choice(index1, size=1, replace=False)
                    choose_samplesb = np.random.choice(index2, size=self.support_number, replace=False)
                    x_input_batch_a[i, j, :, :, :] = self.resize(self.datasets[dataset_name][j][choose_samplesa[0]])
                    y_input_batch_a[i, j, j] = 1
                    y_global_input_batch_a[i, j, j] = 1

                    for k in range(self.support_number):
                        x_input_batch_b[i, self.support_number * j + k, :, :, :] = \
                            self.resize(self.datasets[dataset_name][j][choose_samplesb[k]])
                        y_input_batch_b[i, self.support_number * j + k, j] = 1
                        y_global_input_batch_b[i, self.support_number * j + k, j] = 1
            ...
            training_dataset = self.datasets[dataset_name][:self.general_classification_samples, 3:]
            #### testing for classifier
            testing_number = int(self.general_classification_samples * 0.4)
            testing_dataset = self.datasets[dataset_name][:self.general_classification_samples, :3]
...
class VGGFaceDAGANDataset(DAGANDataset):
    ...
    def load_dataset(self, gan_training_index):
        ...
        self.x = np.load("./Augmentednpy/dataset_forclassifier")
        self.x = self.x / 255
...
class animalsDAGANDataset(DAGANDataset):
    ...
    def load_dataset(self, gan_training_index):
        ...
        self.x = np.load("./Augmentednpy/dataset_forclassifier/animals_128.npy")
...
class flowersDAGANDataset(DAGANDataset):
    ...
    def load_dataset(self, gan_training_index):
        ...
        self.x = np.load("./Augmentednpy/dataset_forclassifier/flowers_520.npy")
