...
def sample_generator(num_generations, sess, same_images, dropout_rate, dropout_rate_value, data, batch_size, file_name,
    ...
    input_images, support_images, generated = data.reconstruct_original(input_images_list), data.reconstruct_original(
        support_list), data.reconstruct_original(generated_list)
    images = np.zeros(
        shape=(batch_size, 1 + support_num + num_generations, support_images.shape[-3], support_images.shape[-2],
               support_images.shape[-1]))
    ...
    x_total_images = np.zeros([batch_size * (1 + support_num + num_generations), image_size, image_size, channel])
    y_total_image = np.zeros([batch_size * (1 + support_num + num_generations), classes_number])
    y_few_shot_total_image = np.zeros([batch_size * (1 + support_num + num_generations), selected_classes])

    for k in range(1 + support_num + num_generations):
        if k == 0:
            ...
            images[:, k] = generated[:, k - 1 - support_num]
            x_total_images[batch_size * k:batch_size * (k + 1)] = generated[:, k - 1 - support_num]
            y_total_image[batch_size * k:batch_size * (k + 1)] = y_label
...
def sample_generator_store_into_file(num_generations, sess, same_images, dropout_rate, dropout_rate_value, data,
    ...
    for j in range(batch_size):
        current_path_quality = file_name.split('//')[0] +'_forquality' + '/{}/'.format(np.argmax(y_global_input_i[:, j]))
        current_path_classifier = file_name.split('//')[0] + '_forclassifier' + '/{}/'.format(np.argmax(y_global_input_i[:, j]))
        ...
        for k in range(1 + support_number + num_generations):
            ##### generating images for evaluating the quality of generated images
            if k > (support_num + 1):
                if (find_classes(current_path_quality) < num_generations):
                    ...
                    current_name = current_path_quality + file_name.split('//')[-1].split('.png')[0] + '_sample{}.png'.format(k)
            ...
            if (find_classes(current_path_classifier) <  (num_generations + support_number +testing_num) ):
                ...
                current_name = current_path_classifier + file_name.split('//')[-1].split('.png')[0] + '_sample{}.png'.format(k)
                scipy.misc.imsave(current_name, current_iamge)
...
def sample_generator_store_into_file_for_quality(num_generations, sess, same_images, dropout_rate, dropout_rate_value, data,
                                     batch_size, file_name,
                                     conditional_inputs, input_a,
                                     support_input, input_b,
                                     y_input_i, input_y_i,
        ...
        for k in range(1 + support_number + num_generations):
            ...
            if k > (support_num + 1):
                if (find_classes(current_path) < num_generations):
                    ...
                    current_name = current_path + file_name.split('//')[-1].split('.png')[0] + '_sample{}.png'.format(k)
...
def sample_generator_store_into_file_for_classifier(num_generations, sess, same_images, dropout_rate, dropout_rate_value, data,
                                     batch_size, file_name,
                                     conditional_inputs, input_a,
                                     support_input, input_b,
                                     y_input_i, input_y_i,
    ...
    for j in range(batch_size):
        ...
        for k in range(1 + support_number + num_generations):
            ##### generating images for calculating
            if (find_classes(current_path) < num_generations + support_number):
                current_iamge = images[image_size * j:image_size * (j + 1), image_size * (k):image_size * (k + 1)]
                if (current_iamge.shape[-2]<128):
                    current_iamge = cv2.resize(current_iamge, (128, 128), interpolation=cv2.INTER_LINEAR)
